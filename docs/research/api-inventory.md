...
Research dates: **2026-08-02**, reviewed **2026-08-04** (including Phase 1F), updated from owner-run
...
9. Owner-run Phase 2B verification used BattleMetrics RCON / Triggers with an owner-controlled,
   RCON-connected server. One native Server Action Webhook action and two automatic Insurgency 2014
   Started Map events each returned HTTP 200 and produced one verified n8n execution. Sanitized
   inspection retained only the configured media type/body and safe verification-field presence plus
   explicit leakage-absence results. No live URL, secret, server/RCON value, request ID, raw headers, or
   execution export was retained. No further external test is required for this phase.
...
| Credential test          | Invalid and valid-subscribed states are verified; valid-unsubscribed and insufficient-scope states remain unresolved                                                    | Read-only `GET /servers` credential test; safe `401`/`403` handling                    |
...
| Permission model         | Live RCON setup exposed Manage Triggers, View Triggers & Exemptions, Manage Trigger Exemptions, Trigger Log, and Use Trigger Webhooks; REST permissions remain separate | **Directly observed** for native trigger setup; exact REST API scope matrix unresolved |
...
| Native triggers    | Configure in RCON / Triggers      | UI-managed; no API endpoint used    | Live setup exposed Server Action, Server Update, Player Join/Update/Leave, and game-specific events including Started Map | Upstream configuration is manual and requires a controlled RCON-connected server for the tested events; automatic registration remains deferred |
| Outbound webhooks  | Configure through trigger actions | **No standalone endpoint verified** | Webhook is a native trigger action; organization roles expose Use Trigger Webhooks                                        | Phase 2B directly verified manual Server Action and automatic Started Map delivery; no API write is implemented                                 |
...
| Outbound webhook   | Receive signed POST               | User-configured n8n webhook URL     | First-party Webhooks help reviewed 2026-08-04; HMAC-SHA256 signs timestamp, period, and exact body                        | Implemented; synthetic exact-byte HTTP proof and Phase 2B real BattleMetrics-originated delivery verified; no polling or registration API       |
