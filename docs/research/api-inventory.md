...
| Subscription             | Anonymous Server collection request returned `403` “Access denied. A subscription is required to use the API.”                                                     | Directly observed on 2026-08-02; applicable plans and token interactions unresolved    |
| Credential test          | No endpoint/response matrix was verified across invalid, valid-unsubscribed, and valid-subscribed tokens                                                           | Network credential test removed; operations validate credentials                       |
...
| Authentication behavior            | No header produced a subscription `403`; an obviously invalid Bearer token produced an invalid/expired-token `401`; valid-token behavior was not tested | **Directly observed** for the first two cases; valid credential states **Unresolved**                                        |
| Subscription requirement           | The anonymous request said an API subscription is required; eligible plans and whether every valid token receives the same gate are unknown             | **Directly observed** gate; plan/token interaction **Unresolved**                                                            |
...
| Offset/cursor/other page parameter | No current page-navigation parameter was accessible                                                                                                     | **Unresolved**; the node constructs none                                                                                     |
| `links.next` form                  | No success response was available, so missing/null/relative/absolute behavior could not be observed                                                     | **Unresolved**; when present, the node accepts only relative or exact-origin HTTPS `/servers` links without user information |
| Successful response media type     | No successful response was available                                                                                                                    | **Unresolved**; the node requests and validates JSON:API                                                                     |
| Error response media type          | Both observed `401` and `403` responses used `application/json`                                                                                         | **Directly observed**                                                                                                        |
| Successful collection envelope     | No success response was available; a JSON:API collection is required by the conservative validator                                                      | Live shape **Unresolved**; JSON:API use is **Inferred** from the first-party API conventions                                 |
...
| Rate-limit headers                 | Neither observed response contained REST quota/remaining/reset headers                                                                                  | Header absence **Directly observed** for these two errors; success/limit behavior **Unresolved**                             |
| `Retry-After` behavior             | Neither observed response contained `Retry-After`; no `429` was deliberately induced                                                                    | Absence **Directly observed** for `401`/`403`; `429` behavior **Unresolved**                                                 |
...
| Live error media type     | `application/json` on observed 401/403 responses, despite requested vendor media type                                   | Directly observed; successful vendor type remains unresolved                                        |
| Observed response headers | `api-version`; request/response identifiers appeared only in a CORS allow-list; neither response exposed quota headers  | Directly observed; headers are handled defensively                                                  |
...
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| Pagination parameters | A probe contained `page[size]`, but authorization failed before support or validation could be established; all remote page fields unresolved |
...
All GET candidates have no request body. A 200 JSON:API single-resource or collection response is the
inferred success shape and requires subscribed live confirmation. The current direct examples are the
Server collection's subscription `403` and invalid-token `401` error documents. Exact rate-limit
behavior is unresolved for every row.
...
| Server: Get Many                  | `GET /servers`                        | [official developer docs](https://www.battlemetrics.com/developers/documentation#link-GET-/servers)                         | Credential required by node; exact scope and valid-token subscription interaction unresolved; anonymous request denied for subscription               | No API query parameters exposed; search/game/country/region/status/sort/include remain unresolved | Follows only returned `links.next`; remote parameters, maximum, and returned link form unresolved | Synthetic mocked test: yes. Live harness: safe, read-only, structural                                  | **Implemented conservatively.** Return All and local Limit only; no authenticated success claimed                                                                      |
...
without a subscription and that an obviously invalid Bearer token is rejected distinctly with `401`.
They do not prove that a Bearer token alone is sufficient, that a valid non-subscribed token is
authenticated before the subscription gate, that all resources behave identically, or that historically
public data is private. A future owner-credential matrix should test, with no captured browser state:

1. a valid personal token without eligible API access;
2. a valid subscribed least-privilege token;
3. a valid token lacking the endpoint's scope;
4. a valid token with scope but without resource/organization permission.
